"""
架构设计工作台
Architecture Designer Workbench

可视化搭建神经网络，实时展示逐层计算
核心理念：让你看到每一层到底算了什么、输出什么形状、占用多少资源
...
def create_layer_from_config(config: LayerConfig, input_shape):
    ...
    try:
        if layer_type == "Conv2d":
            ...
            # 计算输出形状
            B, C, H, W = input_shape
            H_out = (H + 2*params.get('padding', 0) - params['kernel_size']) // params.get('stride', 1) + 1
            W_out = (W + 2*params.get('padding', 0) - params['kernel_size']) // params.get('stride', 1) + 1
            ...
            layer = nn.MaxPool2d(params['kernel_size'], stride=params.get('stride', params['kernel_size']))
            B, C, H, W = input_shape
...
def detect_layer_issues(config: LayerConfig, input_shape, prev_config=None) -> None:
    ...
    if layer_type == "Conv2d":
        ...
        if len(input_shape) != 4:
            ...
            if in_channels != expected_in:
                config.issues.append(f"❌ 通道数不匹配: 输入{in_channels}, 期望{expected_in}")
        ...
        # 检查参数数量
        param_count = params['in_channels'] * params['out_channels'] * (params['kernel_size'] ** 2)
    ...
    # 检测Linear问题
        ...
        if len(input_shape) == 4:
            ...
            if in_features != expected_in:
                config.issues.append(f"❌ 特征数不匹配: 输入{in_features}, 期望{expected_in}")
    ...
    # 检测BatchNorm问题
        ...
        if len(input_shape) != 4:
            ...
            if num_channels != expected:
                config.issues.append(f"❌ 通道数不匹配: 输入{num_channels}, 期望{expected}")
    ...
    # 检测连续的Pooling层
    if layer_type == "MaxPool2d" and prev_config and prev_config.layer_type == "MaxPool2d":
...
def visualize_network_flow(layers_config):
    ...
    for i, config in enumerate(layers_config):
        ...
        fig.add_trace(go.Scatter(
            ...
            y=[y_pos],
            mode='markers+text',
            marker=dict(size=80, color=color, line=dict(color=border_color, width=border_width)),
            text=f"{config.name}",
...
def visualize_activation_heatmap(activation_data):
    ...
    fig.update_layout(
        title=f"{activation_data['name']} - 激活值热力图",
        height=400
    )
...
def architecture_designer_tab(chinese_supported=True):
    ...
    st.header("🎨 架构设计工作台 - 增强版")
    st.markdown("""
    > **核心功能**：可视化搭建神经网络，实时展示每一层的计算细节
    ...
    **新功能**：
    ...
    with col_left:
        ...
        with st.expander("⚙️ 输入配置", expanded=True):
            ...
            input_type = st.selectbox("输入类型", ["图像", "向量"], index=input_type_index, key="input_type_selector")
            
            if input_type == "图像":
                col1, col2 = st.columns(2)
                with col1:
                    # 从当前形状读取默认值
                    default_channels = current_shape[1] if len(current_shape) == 4 else 3
                    channels = st.number_input("通道数", 1, 4, default_channels, key="input_channels")
                with col2:
                    default_size = current_shape[2] if len(current_shape) == 4 else 224
                    size_options = [28, 32, 64, 224]
                    default_index = size_options.index(default_size) if default_size in size_options else 3
                    img_size = st.selectbox("图像尺寸", size_options, index=default_index, key="input_size")
                st.session_state.input_shape = (1, channels, img_size, img_size)
            else:
                # 从当前形状读取默认值
                default_vector_size = current_shape[1] if len(current_shape) == 2 else 784
                vector_size = st.number_input("向量维度", 1, 100000, default_vector_size, key="vector_size")
        ...
        with st.expander("➕ 添加层", expanded=True):
            layer_type = st.selectbox(
                "选择层类型",
                ["Conv2d", "Linear", "ReLU", "MaxPool2d", "Flatten", "BatchNorm2d", "Dropout"]
            )
            
            layer_name = st.text_input("层名称", f"{layer_type}_{len(st.session_state.layers)+1}")
            ...
            if layer_type == "Conv2d":
                ...
                with col1:
                    ...
                    if len(st.session_state.layers) == 0:
                        ...
                        if prev_layer.layer_type == "Conv2d":
                            default_in_channels = prev_layer.params.get('out_channels', 64)
                        elif prev_layer.layer_type == "BatchNorm2d":
                            default_in_channels = prev_layer.params.get('num_features', 64)
                    ...
                    params['in_channels'] = st.number_input("输入通道", 1, 512, default_in_channels)
                ...
                # 显示提示信息
                if len(st.session_state.layers) > 0 and st.session_state.layers[-1].layer_type == "Flatten":
                    st.info(f"💡 Flatten后特征数: {default_in_features:,}")
                
                params['in_features'] = st.number_input("输入特征", 1, 10000000, default_in_features)
                ...
                if len(st.session_state.layers) > 0:
                    ...
                    default_num_features = 64
                params['num_features'] = st.number_input("特征数", 1, 512, default_num_features)
        ...
        if st.session_state.layers:
            with st.expander("📋 层管理", expanded=True):
                for i, layer in enumerate(st.session_state.layers):
                    ...
                    with col1:
                        # 显示层名和状态图标
                        status_icon = "❌" if layer.has_issues else ("⚠️" if layer.warnings else "✅")
                        st.write(f"{i+1}. {status_icon} {layer.name} ({layer.layer_type})")
                    with col2:
                        if i > 0 and st.button("⬆️", key=f"up_{i}", help="上移"):
                            ...
                            st.rerun()
                    with col3:
                        if i < len(st.session_state.layers) - 1 and st.button("⬇️", key=f"down_{i}", help="下移"):
                            st.session_state.layers[i], st.session_state.layers[i+1] = \
                                st.session_state.layers[i+1], st.session_state.layers[i]
                            st.rerun()
                    with col4:
        ...
        with st.expander("💾 导入/导出", expanded=False):
            st.markdown("**导出配置**")
            if st.session_state.layers:
                config_json = export_network_config(st.session_state.layers, st.session_state.input_shape)
                st.download_button(
                    label="📥 下载配置文件",
                    data=config_json,
    ...
    with col_right:
        ...
        if not st.session_state.layers:
            ...
            if has_any_issues or has_any_warnings:
                ...
                warning_count = sum(1 for c in st.session_state.layers if c.warnings and not c.has_issues)
                ...
                with col3:
                    if issue_count > 0 and st.button("🔧 一键修复", type="primary", help="自动修正所有形状不匹配问题"):
                        ...
                        for idx, config in enumerate(st.session_state.layers):
                            ...
                            if config.layer_type == "Linear" and len(temp_shape) == 4:
                                # 需要在Linear前插入Flatten
                                flatten_config = LayerConfig("Flatten", f"flatten_auto_{idx}", {})
                                flatten_layer = create_layer_from_config(flatten_config, temp_shape)
                            ...
                            elif config.layer_type == "BatchNorm2d" and len(temp_shape) == 4:
                ...
                for i, config in enumerate(st.session_state.layers):
                    if config.issues or config.warnings:
                        with st.expander(f"{'❌' if config.has_issues else '⚠️'} {config.name} - 问题详情", expanded=config.has_issues):
                            if config.issues:
                                st.markdown("**错误：**")
            ...
            if valid_network:
                ...
                total_params = sum(config.param_count for config in st.session_state.layers)
                ...
                if st.checkbox("📊 显示详细层信息表", value=False):
                    ...
                    table_data = []
                    for i, config in enumerate(st.session_state.layers):
                        status = "❌ 错误" if config.has_issues else ("⚠️ 警告" if config.warnings else "✅ 正常")
                        table_data.append({
                            "序号": i+1,
                            "层名": config.name,
                            "类型": config.layer_type,
                            "输出形状": str(config.output_shape) if config.output_shape else "N/A",
                ...
                # 运行前检查是否有错误
                has_errors_before_run = any(c.has_issues for c in st.session_state.layers)
                
                if has_errors_before_run:
                    st.error("⚠️ 网络中存在错误，无法运行前向传播！请先修复错误。")
                    if st.button("🔧 自动修复并运行", type="primary", key="auto_fix_run"):
                        ...
                        st.success(f"✅ 自动修复完成！已修正 {fixed_count} 个问题，正在运行...")
                        st.rerun()
                    ...
                    with st.spinner("计算中..."):
                        try:
                            ...
                            # 前向传播
                            output, activations = simulate_forward_pass(model, input_data)
                            ...
                            if output.numel() <= 100:
                                ...
                                st.write(f"输出统计: mean={output.mean():.4f}, std={output.std():.4f}, min={output.min():.4f}, max={output.max():.4f}")
                            ...
                            if activations:
                                ...
                                for act in activations:
                                    with st.expander(f"📍 {act['name']} - 形状: {act['shape']}", expanded=False):
                                        col1, col2 = st.columns(2)
                                        ...
                                        with col2:
                                            ...
                                            st.write(f"元素数量: {np.prod(act['shape']):,}")
