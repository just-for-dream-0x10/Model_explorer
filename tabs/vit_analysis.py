...
def visualize_patch_embedding(img_size=224, patch_size=16):
    ...
    fig.update_layout(
        title=f"Patch Embedding: {img_size}×{img_size} 图像切分成 {num_patches} 个 {patch_size}×{patch_size} patches",
...
def vit_analysis_tab(chinese_supported=True):
    ...
    **验证方法**：可视化Patch Embedding、Self-Attention，对比ViT和CNN
    ...
    st.info(
        f"""
    **计算过程**：
    - 图像尺寸: {img_size}×{img_size}
    - Patch大小: {patch_size}×{patch_size}
    - Patches数量: ({img_size}÷{patch_size})² = **{num_patches}个**
    - 每个patch维度: {patch_size}×{patch_size}×3 = {patch_size*patch_size*3}
    """
    )
...
Patch大小: {config['patch_size']}×{config['patch_size']}
...
Transformer Blocks:
  ...
  {arch['attention']}
    ...
    **时间复杂度**：
    ```
    Q @ K^T: O(N² · d)    # N×d 矩阵乘以 d×N 矩阵
    Softmax: O(N²)        # 对N×N矩阵做softmax
    Attn @ V: O(N² · d)   # N×N 矩阵乘以 N×d 矩阵
    ...
    **空间复杂度**：
    ...
    | 操作 | 复杂度 | 说明 |
    |------|--------|------|
    | **Self-Attention** | O(N²·d) | N=patches数量，随图像尺寸平方增长 |
    | **卷积** | O(k²·d²·N) | k=kernel大小，d=通道数，N=特征图大小 |
    
    ### 实际数值
    
    假设224×224图像，patch_size=16：
    - N = (224÷16)² = 196个patches
    - Attention矩阵: 196×196 = 38,416个元素
    - 12个头: 12×38,416 = 460,992个元素
    
    **结论**：
    - ViT的计算量随图像尺寸平方增长
    - 高分辨率图像（如1024×1024）计算量巨大
    ...
    **如果你是...**
    ...
    - **196**: 224×224图像使用16×16 patch得到的序列长度
