...
def backpropagation_tab(CHINESE_SUPPORTED):
    ...
    with st.expander("💡 反向传播核心概念", expanded=True):
        st.markdown(
            """
        **反向传播算法 (Backpropagation)** - 数值计算过程详解
        ...
        **第1步：前向传播的数值计算**
        ...
        # 示例：简单全连接网络
        输入: x = [0.5, -0.3]
        权重1: W1 = [[0.2, 0.1], [-0.1, 0.3]]
        偏置1: b1 = [0.1, -0.1]
        
        # 隐藏层计算
        z1 = x · W1 + b1 = [0.5×0.2 + (-0.3)×(-0.1) + 0.1, 0.5×0.1 + (-0.3)×0.3 - 0.1]
             = [0.1 + 0.03 + 0.1, 0.05 - 0.09 - 0.1] = [0.23, -0.14]
        
        a1 = ReLU(z1) = [max(0,0.23), max(0,-0.14)] = [0.23, 0]
        
        # 输出层计算  
        W2 = [[0.4, -0.2], [0.1, 0.3]]
        b2 = [0.05, 0.02]
        
        z2 = a1 · W2 + b2 = [0.23×0.4 + 0×(-0.2) + 0.05, 0.23×0.1 + 0×0.3 + 0.02]
             = [0.092 + 0.05, 0.023 + 0.02] = [0.142, 0.043]
        
        y = softmax(z2) = [0.536, 0.464]
        ```
        
        **第2步：损失函数的数值计算**
        ```
        真实标签: t = [1, 0]
        交叉熵损失: L = -Σ(t[i] × log(y[i]))
                     = -(1 × log(0.536) + 0 × log(0.464))
                     = -(-0.624) = 0.624
        ```
        ...
        **第3步：反向传播的梯度计算**
        ```
        # 输出层梯度
        ∂L/∂z2 = y - t = [0.536-1, 0.464-0] = [-0.464, 0.464]
        
        # 输出层权重梯度
        ∂L/∂W2 = a1^T · ∂L/∂z2 = [[0.23], [0]] · [-0.464, 0.464]
                  = [[-0.107, 0.107], [0, 0]]
        
        # 隐藏层梯度 (关键：为什么第二个神经元梯度为0？)
        ∂L/∂a1 = ∂L/∂z2 · W2^T = [-0.464, 0.464] · [[0.4, 0.1], [-0.2, 0.3]]
                   = [-0.279, 0.093]
        
        # ReLU导数 (关键：被置零的神经元没有梯度)
        ∂a1/∂z1 = [1 if z1[i] > 0 else 0] = [1, 0]
        
        ∂L/∂z1 = ∂L/∂a1 ⊙ ∂a1/∂z1 = [-0.279×1, 0.093×0] = [-0.279, 0]
        
        # 隐藏层权重梯度
        ∂L/∂W1 = x^T · ∂L/∂z1 = [[0.5], [-0.3]] · [-0.279, 0]
                  = [[-0.140, 0], [0.084, 0]]
        ```
        ...
        **第4步：参数更新的数值变化**
        ...
        W2_new = W2 - α × ∂L/∂W2 = [[0.401, -0.201], [0.1, 0.3]]
        W1_new = W1 - α × ∂L/∂W1 = [[0.201, 0.1], [-0.101, 0.3]]
