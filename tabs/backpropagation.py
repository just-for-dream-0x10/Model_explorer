...
def backpropagation_tab(CHINESE_SUPPORTED):
    ...
    with st.expander("💡 反向传播核心概念", expanded=True):
        ...
        **数学基础**：
        ...
        display_latex(r"\frac{\partial L}{\partial w_i} = \frac{\partial L}{\partial y} \cdot \frac{\partial y}{\partial z} \cdot \frac{\partial z}{\partial w_i}")
...
def _fcn_backprop(CHINESE_SUPPORTED):
    ...
    **数值梯度法验证解析梯度的正确性：**
    ...
    display_latex(r"\frac{\partial L}{\partial w} \approx \frac{L(w + \epsilon) - L(w - \epsilon)}{2\epsilon}")
...
def _cnn_backprop(CHINESE_SUPPORTED):
    ...
    **卷积层反向传播的关键：**
    ...
    with col1:
        ...
        display_latex(r"\frac{\partial L}{\partial K} = X * \frac{\partial L}{\partial Y}")
        st.markdown("其中 $*$ 表示卷积运算")
    
    with col2:
        st.markdown("**损失对输入的梯度：**")
        display_latex(r"\frac{\partial L}{\partial X} = \text{rot}_{180}(K) * \frac{\partial L}{\partial Y}")
    ...
    with col2:
        ...
        st.markdown(f"**损失对输出的梯度** $\\frac{{\\partial L}}{{\\partial Y}}$ ({output_size}×{output_size}):")
        st.dataframe(pd.DataFrame(dL_dy.round(3)), width=250)
        
        st.markdown(f"**损失对卷积核的梯度** $\\frac{{\\partial L}}{{\\partial K}}$ ({kernel_size}×{kernel_size}):")
        st.dataframe(pd.DataFrame(dL_dK.round(3)), width=250)
        
        st.markdown(f"**损失对输入的梯度** $\\frac{{\\partial L}}{{\\partial X}}$ ({input_size}×{input_size}):")
    ...
    fig.add_trace(
        go.Heatmap(z=dL_dx, colorscale='RdBu', zmid=0),
        row=1, col=1
    )
    
    fig.add_trace(
        go.Heatmap(z=dL_dK, colorscale='RdBu', zmid=0),
        row=1, col=2
    )
    
    fig.add_trace(
        go.Heatmap(z=dL_dy, colorscale='RdBu', zmid=0),
        row=1, col=3
    )
...
def _rnn_backprop(CHINESE_SUPPORTED):
    ...
    **关键挑战：**
    ...
    with col2:
        st.markdown("**反向传播：**")
        display_latex(r"\frac{\partial L}{\partial h_t} = \frac{\partial L_t}{\partial h_t} + \frac{\partial L}{\partial h_{t+1}} \cdot \frac{\partial h_{t+1}}{\partial h_t}")
    ...
    **梯度在时间上的传播：**
    ...
    with col2:
        weight_eigenvalue = st.slider("权重矩阵特征值", 0.3, 2.0, 1.0, step=0.1, key="rnn_eigenvalue")
    ...
    if weight_eigenvalue > 1.1:
        ...
        st.warning(f"⚠️ **梯度消失风险！** 特征值 < 1，梯度从 1.0 衰减到 {final_grad:.2e}")
