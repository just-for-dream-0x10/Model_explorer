...
总参数量 = {result['parameters']['weight']:,} + {result['parameters']['bias']}
        ...
        st.latex(
            r"MACs = K_h \times K_w \times C_{in} \times C_{out} \times H_{out} \times W_{out}"
        )
        st.latex(
            r"FLOPs = 2 \times MACs"
            + (r" + C_{out} \times H_{out} \times W_{out}" if use_bias else "")
        )
...
总 FLOPs (1 MAC = 2 FLOPs):
    ...
    = {result['flops']['total']:,}
        ...
        fig = go.Figure(
            data=[
                go.Pie(
                    ...
                    values=(
                        [result["parameters"]["weight"], result["parameters"]["bias"]]
                        if use_bias
                        else [result["parameters"]["weight"]]
                    ),
                    hole=0.3,
                )
            ]
        ...
        col1, col2 = st.columns(2)
        with col1:
            in_features = st.number_input(
                "输入特征数 (in_features)", min_value=1, value=512, step=1
            )
        with col2:
            out_features = st.number_input(
                "输出特征数 (out_features)", min_value=1, value=1000, step=1
            )
        ...
        with col1:
            num_features = st.number_input(
                "通道数 (num_features)", min_value=1, value=64, step=1
            )
        with col2:
        ...
        st.markdown("#### 参数量")
        st.latex(
            r"Params_{total} = 2 \times num\_features \quad (\gamma \text{ 和 } \beta)"
        )
        ...
        with col1:
            st.markdown("**输入配置**")
            C_in = st.number_input(
                "输入通道数", min_value=1, value=64, step=1, key="dw_cin"
            )
            H_in = st.number_input(
                "输入高度", min_value=1, value=56, step=1, key="dw_hin"
            )
            W_in = st.number_input(
                "输入宽度", min_value=1, value=56, step=1, key="dw_win"
            )

        with col2:
            st.markdown("**层参数配置**")
            kernel_size = st.number_input(
                "卷积核大小", min_value=1, value=3, step=1, key="dw_kernel"
            )
            stride = st.number_input(
                "步长", min_value=1, value=1, step=1, key="dw_stride"
            )
            padding = st.number_input(
                "填充", min_value=0, value=1, step=1, key="dw_padding"
            )
        ...
        with col1:
            ...
            st.metric(
                "参数减少", f"{reduction:.1f}%", delta=f"-{std_params - dw_params:,}"
            )
        ...
        **优化建议**:
        ...
        with col1:
            ...
            input_size = st.number_input(
                "输入维度", min_value=1, value=128, step=1, key="lstm_in"
            )
            hidden_size = st.number_input(
                "隐藏层维度", min_value=1, value=256, step=1, key="lstm_hidden"
            )

        with col2:
            st.markdown("**层配置**")
            num_layers = st.number_input(
                "层数", min_value=1, max_value=10, value=2, step=1, key="lstm_layers"
            )
        ...
        result = analyzer.lstm_analysis(
            input_size, hidden_size, num_layers, use_bias, bidirectional
        )
        ...
        with col2:
            ...
            seq_length = st.number_input(
                "序列长度", min_value=1, value=50, step=1, key="lstm_seqlen"
            )
        ...
        st.latex(
            r"Params_{layer1} = 4 \times (input\_size \times hidden\_size + hidden\_size^2)"
        )

        if num_layers > 1:
            input_size_other = hidden_size * (2 if bidirectional else 1)
            st.latex(
                r"Params_{other} = 4 \times (hidden\_size \times num\_directions \times hidden\_size + hidden\_size^2)"
            )
...
总参数量: {result['parameters']['total']:,}
        ...
        if st.checkbox("与GRU对比", key="lstm_compare_gru"):
            gru_params = (
                num_layers * 3 * (input_size * hidden_size + hidden_size * hidden_size)
            )
            if bidirectional:
                gru_params *= 2
        ...
        with col1:
            st.metric("总参数量", f"{result['parameters']['total']:,}")
            readable = (
                f"{result['parameters']['total']/1e6:.2f}M"
                if result["parameters"]["total"] > 1e6
                else f"{result['parameters']['total']/1e3:.2f}K"
            )
...
{num_embeddings:,} × {embedding_dim} = {result['parameters']['total']:,}
        ...
        with col2:
            batch_size = st.number_input(
                "批次大小", min_value=1, value=32, step=1, key="ln_batch"
            )
            seq_len = st.number_input(
                "序列长度", min_value=1, value=128, step=1, key="ln_seq"
            )
...
def _predefined_network_analysis():
    ...
    with col4:
        st.metric("激活内存", f"{total_memory:.2f}MB", help="前向传播激活值占用的内存")
    ...
    # 格式化显示
    df["params_readable"] = df["params"].apply(
        lambda x: f"{x/1e6:.2f}M" if x > 1e6 else f"{x/1e3:.2f}K"
    )
    df["flops_readable"] = df["flops"].apply(
    ...
    with col1:
        fig1 = go.Figure(
            data=[go.Bar(x=df["layer_name"], y=df["params"], marker_color="lightblue")]
        )
        fig1.update_layout(
            title="各层参数量分布",
            xaxis_title="层名称",
    ...
    with col2:
        fig2 = go.Figure(
            data=[go.Bar(x=df["layer_name"], y=df["flops"], marker_color="lightcoral")]
        )
        fig2.update_layout(
            title="各层FLOPs分布", xaxis_title="层名称", yaxis_title="FLOPs", height=400
        )
    ...
    # 调用诊断功能（第4个核心问题）
    _add_diagnostics_section(total_params, total_flops)
    ...
    if st.button("生成Markdown报告", use_container_width=True):
        report = _generate_network_report(
            network_name, input_size, batch_size, layers_data, total_params, total_flops
        )
    ...
    **快速构建自定义网络并分析参数量。**
    ...
    with col1:
        input_channels = st.number_input(
            "输入通道数", min_value=1, value=3, step=1, key="custom_input_c"
        )
    with col2:
        input_height = st.number_input(
            "输入高度", min_value=1, value=224, step=1, key="custom_input_h"
        )
    with col3:
        input_width = st.number_input(
            "输入宽度", min_value=1, value=224, step=1, key="custom_input_w"
        )
                    ...
                    if layer["type"] == "Conv2d":
                        ...
                        with col_a:
                            out_channels = st.number_input(
                                "输出通道", min_value=1, value=64, key=f"conv_out_{idx}"
                            )
                        with col_b:
                            kernel = st.number_input(
                                "卷积核", min_value=1, value=3, key=f"conv_k_{idx}"
                            )
                        with col_c:
                            stride = st.number_input(
                                "步长", min_value=1, value=1, key=f"conv_s_{idx}"
                            )
                        with col_d:
                            padding = st.number_input(
                                "填充", min_value=0, value=1, key=f"conv_p_{idx}"
                            )
                        ...
                        with col_a:
                            pool_kernel = st.number_input(
                                "池化核", min_value=1, value=2, key=f"pool_k_{idx}"
                            )
                        with col_b:
                            pool_stride = st.number_input(
                                "池化步长", min_value=1, value=2, key=f"pool_s_{idx}"
                            )
                        ...
                        if len(current_shape) == 3:
                            result = analyzer.batchnorm2d_analysis(
                                current_shape[0], current_shape
                            )
                            total_params += result["parameters"]["total"]
                            total_flops += result["flops"]["total"]
...
def _get_network_config(network_name: str, input_size: int) -> List[Dict]:
    ...
    if "ResNet-18" in network_name:
        return [
            {
                "layer_name": "conv1",
                "layer_type": "Conv2d",
                "output_shape": (64, input_size // 2, input_size // 2),
                "params": 9408,
                "flops": 118013952,
            },
            {
            ...
            },
            {
                "layer_name": "layer2",
                "layer_type": "ResBlock",
                "output_shape": (128, input_size // 4, input_size // 4),
                "params": 525568,
                "flops": 924844032,
            },
            {
        ...
        return [
            {
                "layer_name": "conv1_1",
                "layer_type": "Conv2d",
                "output_shape": (64, input_size, input_size),
                "params": 1792,
                "flops": 86704128,
            },
            {
            ...
            },
            {
                "layer_name": "conv2_1",
                "layer_type": "Conv2d",
                "output_shape": (128, input_size // 2, input_size // 2),
                "params": 73856,
                "flops": 924844032,
            },
            {
        ...
        return [
            ...
            {
                "layer_name": "bottleneck1",
                "layer_type": "InvertedResidual",
                "output_shape": (16, input_size // 2, input_size // 2),
                "params": 896,
                "flops": 11239424,
            },
            {
                "layer_name": "bottleneck2",
                "layer_type": "InvertedResidual",
                "output_shape": (24, input_size // 4, input_size // 4),
                "params": 5136,
                "flops": 40140800,
            },
            {
                "layer_name": "bottleneck3",
                "layer_type": "InvertedResidual",
                "output_shape": (32, input_size // 8, input_size // 8),
                "params": 8832,
                "flops": 34406400,
            },
            {
                "layer_name": "bottleneck4",
                "layer_type": "InvertedResidual",
                "output_shape": (64, input_size // 16, input_size // 16),
                "params": 25728,
                "flops": 50135040,
            },
            {
                "layer_name": "bottleneck5",
                "layer_type": "InvertedResidual",
                "output_shape": (96, input_size // 16, input_size // 16),
                "params": 66624,
                "flops": 129957888,
            },
            {
                "layer_name": "bottleneck6",
                "layer_type": "InvertedResidual",
                "output_shape": (160, input_size // 32, input_size // 32),
                "params": 118272,
                "flops": 91570176,
            },
            {
                "layer_name": "bottleneck7",
                "layer_type": "InvertedResidual",
                "output_shape": (320, input_size // 32, input_size // 32),
                "params": 155264,
                "flops": 120197120,
            },
            {
                ...
                "layer_type": "Conv2d",
                "output_shape": (1280, input_size // 32, input_size // 32),
                "params": 409600,
                "flops": 200704000,
            },
            {
        ...
        for i in range(12):
            ...
            attn_flops = (
                6 * seq_len * d_model * d_model
                + 2 * num_heads * seq_len * seq_len * (d_model // num_heads) * 2
            )
            ...
            ffn_params = (
                d_model * ffn_hidden + ffn_hidden + ffn_hidden * d_model + d_model
            )
...
"""
    ...
    for layer in layers_data:
        params_str = (
            f"{layer['params']/1e6:.2f}M"
            if layer["params"] > 1e6
            else f"{layer['params']:,}"
        )
        flops_str = (
            f"{layer['flops']/1e9:.2f}G"
            if layer["flops"] > 1e9
            else f"{layer['flops']/1e6:.2f}M"
        )
...
def _add_diagnostics_section(total_params, total_flops, layer_type=None):
    """
    添加统一的参数量/FLOPs诊断部分（第4个核心问题）
    """
    st.markdown("---")
    st.markdown("### ⚠️ 参数量/FLOPs诊断")
    ...
    if total_params > 100e6:
        issues.append(
            {
                "问题": "参数量过大",
                "检测结果": f"{total_params/1e6:.1f}M参数（>100M）",
                "影响": "GPU显存不足、训练速度慢、部署困难",
                "解决方案": "模型压缩（剪枝/量化）、知识蒸馏、或使用更小的模型",
            }
        )
        ...
        warnings.append(
            {
                "问题": "参数量较大",
                "检测结果": f"{total_params/1e6:.1f}M参数（50M-100M）",
                "影响": "训练和推理开销大",
                "解决方案": "考虑模型压缩或使用更高效的架构",
            }
        )
    ...
    if total_flops > 10e9:
        warnings.append(
            {
                "问题": "FLOPs过高",
                "检测结果": f"{total_flops/1e9:.1f}G FLOPs（>10G）",
                "影响": "推理速度慢，移动端部署困难",
                "解决方案": "深度可分离卷积、轻量化架构（MobileNet/EfficientNet）",
            }
        )

    # 检测3: 参数效率低
    if total_params > 0 and total_flops > 0:
        param_efficiency = total_flops / total_params
        if param_efficiency < 100:
            warnings.append(
                {
                    "问题": "参数效率低",
                    "检测结果": f"FLOPs/Params比值={param_efficiency:.1f}（<100）",
                    "影响": "参数多但计算少，可能存在冗余",
                    "解决方案": "减少全连接层、使用更深的卷积",
                }
            )
    ...
    if layer_type == "MultiHeadAttention":
        ...
        if total_params > 10e6:
            warnings.append(
                {
                    "问题": "全连接层参数过多",
                    "检测结果": f"Linear层{total_params/1e6:.1f}M参数",
                    "影响": "参数冗余、容易过拟合",
                    "解决方案": "Global Average Pooling替代、或添加Dropout",
                }
            )
    ...
    if not issues and not warnings:
        st.success("✅ 参数量和计算量在合理范围内！")

    # 参考基准表格
    st.markdown("---")
    st.markdown("### 📊 模型规模参考基准")

    benchmark_table = pd.DataFrame(
        {
            "模型规模": ["微型", "小型", "中型", "大型", "超大型"],
            "参数量": ["<1M", "1M-10M", "10M-100M", "100M-1B", ">1B"],
            "典型例子": [
                "MobileNet-V3 small",
                "ResNet-18, BERT-tiny",
                "ResNet-50, BERT-base",
    ...
    # 权衡分析
    st.markdown("---")
    st.markdown("### ⚖️ 优化权衡分析")

    tradeoff_table = pd.DataFrame(
        {
            "优化方法": ["剪枝", "量化", "蒸馏", "架构搜索", "深度可分离卷积"],
            "参数减少": ["⭐⭐⭐⭐", "⭐", "⭐⭐⭐⭐⭐", "⭐⭐⭐", "⭐⭐⭐"],
            "FLOPs减少": ["⭐⭐⭐", "⭐", "⭐⭐⭐⭐⭐", "⭐⭐⭐⭐", "⭐⭐⭐⭐⭐"],
            "精度保持": ["⭐⭐⭐", "⭐⭐⭐⭐", "⭐⭐⭐⭐⭐", "⭐⭐⭐⭐", "⭐⭐⭐⭐"],
            "实施难度": ["中", "低", "高", "高", "低"],
    ...
    1. **评估需求**：明确部署环境（云/边缘/移动）和性能要求
    2. **选择策略**：参数多→剪枝/蒸馏；FLOPs高→深度可分离卷积
    3. **逐步优化**：先架构改进，再剪枝，最后量化
    4. **持续评估**：每步优化后检查精度损失
    5. **A/B测试**：在实际场景中验证效果
    """
    )
