...
def plot_gradient_flow(gradient_stats):
    ...
    fig.add_hline(y=1e2, line_dash="dash", line_color="purple", 
        ...
        if st.button("🚀 开始梯度分析", key="grad_analysis"):
            ...
            if gradient_stats:
                ...
                for i, stat in enumerate(display_stats):
                    mean_grad = stat['mean']
                    if mean_grad < 1e-5:
                        st.error(f"❌ {stat['layer']}: 梯度={mean_grad:.2e} (严重消失！)")
                    elif mean_grad < 1e-3:
                        st.warning(f"⚠️ {stat['layer']}: 梯度={mean_grad:.2e} (轻微消失)")
                    else:
...
参数量 = 64 × 1000 + 1000 = 65,000 (减少了5万倍！)
        ...
        st.info("💡 提示：由于时间限制，这里展示理论分析。实际训练可以看到Loss剧烈震荡。")
