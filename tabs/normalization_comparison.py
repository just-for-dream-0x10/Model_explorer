...
def plot_normalization_comparison(original, batch_norm, layer_norm, group_norm):
    ...
    # 原始
    orig_sample = np.random.choice(
        original.flatten(), size=min(sample_size, original.size), replace=False
    )
    fig.add_trace(
        go.Histogram(
            x=orig_sample, nbinsx=30, name="原始", marker_color="gray", opacity=0.7
        ),
        row=1,
        col=1,
    )
    ...
    # BatchNorm
    bn_sample = np.random.choice(
        batch_norm.flatten(), size=min(sample_size, batch_norm.size), replace=False
    )
    fig.add_trace(
        go.Histogram(
            x=bn_sample, nbinsx=30, name="BatchNorm", marker_color="red", opacity=0.7
        ),
        row=1,
        col=2,
    )
    ...
    # LayerNorm
    ln_sample = np.random.choice(
        layer_norm.flatten(), size=min(sample_size, layer_norm.size), replace=False
    )
    fig.add_trace(
        go.Histogram(
            x=ln_sample, nbinsx=30, name="LayerNorm", marker_color="green", opacity=0.7
        ),
        row=2,
        col=1,
    )
    ...
    # GroupNorm
    gn_sample = np.random.choice(
        group_norm.flatten(), size=min(sample_size, group_norm.size), replace=False
    )
    fig.add_trace(
        go.Histogram(
            x=gn_sample, nbinsx=30, name="GroupNorm", marker_color="blue", opacity=0.7
        ),
        row=2,
        col=2,
    )

    fig.update_layout(title="归一化方法对比", height=600, showlegend=False)

    return fig


def plot_channel_statistics(
    original, batch_norm, layer_norm, group_norm, num_channels=16
):
    """
    绘制每个通道的均值和标准差
    ...
    # 均值对比
    fig = make_subplots(
        rows=1, cols=2, subplot_titles=("各通道均值对比", "各通道标准差对比")
    )

    # 均值
    fig.add_trace(
        go.Bar(x=channels, y=orig_means, name="原始", marker_color="gray"), row=1, col=1
    )
    fig.add_trace(
    ...
    fig.add_trace(
        go.Bar(
            x=channels, y=orig_stds, name="原始", marker_color="gray", showlegend=False
        ),
        row=1,
        col=2,
    )
    ...
    fig.update_layout(title="通道统计量对比", height=400, barmode="group")
    ...
    **验证方法**：用可视化展示"在哪个维度归一化"的实际效果
    ...
    with col1:
        batch_size = st.slider(
            "Batch Size", 1, 8, 4, 1, help="批次大小，BatchNorm依赖此参数"
        )

    with col2:
        num_channels = st.slider("通道数", 16, 64, 32, 16, help="特征通道数")

    with col3:
        num_groups = st.slider(
            "GroupNorm分组数", 4, 16, 8, 4, help="GroupNorm的分组数量"
        )
    ...
    if st.button("🚀 生成数据并对比", type="primary"):
        with st.spinner("生成数据并计算..."):
            ...
            **原始数据统计**：
            ...
            st.success(
                "✅ 观察：归一化后，所有方法都将激活值分布调整到均值≈0、标准差≈1"
            )
    ...
    **关键问题**：为什么BatchNorm在小batch场景下效果差？
    ...
    if st.button("🧪 运行Batch Size敏感性测试"):
        with st.spinner("测试中..."):
            ...
            fig.add_hline(
                y=1.0, line_dash="dash", line_color="gray", annotation_text="理想值=1.0"
            )
