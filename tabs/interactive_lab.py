...
def _cnn_feature_visualization(CHINESE_SUPPORTED):
    ...
    # 图像输入选项
    st.markdown("#### 📁 图像输入方式")
    input_method = st.radio("选择输入方式", ["上传真实图像", "使用示例图像"], key="cnn_input_method")
    ...
    if input_method == "上传真实图像":
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                st.markdown(f"**原始图像尺寸**: {original_size[0]} × {original_size[1]}")
                ...
                # 调整大小以便处理
                target_size = st.slider("处理尺寸", 32, 256, 64, step=32, key="cnn_target_size")
    ...
    if input_image is not None:
        ...
        # 添加padding
        if padding > 0:
            padded_input = np.pad(input_image, padding, mode='constant', constant_values=0)
        else:
        ...
        # 激活函数
        activation = st.selectbox("激活函数", ["None", "ReLU", "Sigmoid", "Tanh"], key="cnn_activation")
        ...
        display_latex(r"H_{out} = \left\lfloor \frac{H_{in} + 2P - K}{S} \right\rfloor + 1")
...
def _activation_comparison(CHINESE_SUPPORTED):
    ...
    with col2:
        test_activation = st.selectbox("测试激活函数", list(activation_funcs.keys()), key="act_test")
    ...
    fig.add_trace(
        go.Scatter(x=list(range(len(values))), y=values, mode='lines+markers', name='激活值'),
        row=1, col=1
    )
    
    fig.add_trace(
        go.Scatter(x=list(range(len(gradients))), y=gradients, mode='lines+markers', name='梯度'),
        row=1, col=2
    )
...
def _optimizer_trajectory(CHINESE_SUPPORTED):
    ...
    with col1:
        loss_type = st.selectbox("损失函数", ["Bowl", "Rosenbrock", "Beale"], key="opt_loss")
    with col2:
    ...
    def optimize_sgd(start, lr, steps=50):
        ...
        for _ in range(steps):
            grad = np.array([
                (loss_func(pos[0]+1e-5, pos[1]) - loss_func(pos[0]-1e-5, pos[1])) / (2e-5),
                (loss_func(pos[0], pos[1]+1e-5) - loss_func(pos[0], pos[1]-1e-5)) / (2e-5)
            ])
            pos = pos - lr * grad
...
def _loss_landscape_3d(CHINESE_SUPPORTED):
    ...
    with col1:
        loss_type = st.selectbox("损失函数", ["Sphere", "Rosenbrock", "Himmelblau", "Rastrigin"], key="loss_type")
    with col2:
...
def _batch_parameter_comparison(CHINESE_SUPPORTED):
    ...
    for i, val in enumerate(param_values):
        ...
        if param_type == "学习率":
            # 学习率影响收敛速度和稳定性
            if val < 0.001:
                train_loss = 2.0 * np.exp(-0.02 * np.arange(epochs)) + np.random.randn(epochs) * 0.05
            elif val < 0.01:
                train_loss = 2.0 * np.exp(-0.05 * np.arange(epochs)) + np.random.randn(epochs) * 0.03
            else:
                train_loss = 2.0 * np.exp(-0.03 * np.arange(epochs)) + np.random.randn(epochs) * 0.1
        else:
            # 其他参数的简化模拟
            decay_rate = 0.03 + np.random.rand() * 0.02
            train_loss = 2.0 * np.exp(-decay_rate * np.arange(epochs)) + np.random.randn(epochs) * 0.05
    ...
    with col1:
        fig2 = go.Figure(data=[
            go.Bar(
                x=[f"{val:.4f}" if isinstance(val, float) else str(int(val)) for val in final_losses.keys()],
                y=list(final_losses.values()),
                marker_color=['green' if val == best_param else 'lightblue' for val in final_losses.keys()]
            )
        ])
    ...
    with col2:
        ...
        improvement = (max(final_losses.values()) - min(final_losses.values())) / max(final_losses.values()) * 100
