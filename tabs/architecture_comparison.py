...
def plot_model_comparison_bars(models_info):
    ...
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=("参数量 (Million)", "FLOPs (GFLOPs)")
    )
    
    model_names = list(models_info.keys())
    params = [info['params'] for info in models_info.values()]
    flops = [info['flops'] for info in models_info.values()]
    colors = ['red' if info['type'] == 'CNN' else 'blue' for info in models_info.values()]
    ...
    fig.update_layout(
        title="模型复杂度对比（红色=CNN，蓝色=Transformer）",
        height=400
    )
    ...
    if st.button("🚀 运行对比实验", type="primary"):
        with st.spinner("生成实验数据..."):
            ...
            for model_name, info in models_info.items():
                ...
                curves = generate_training_curve(
                    model_type, 
                    dataset_size, 
        ...
        if show_acc:
            ...
            for model_name, curves in curves_dict.items():
                ...
                st.write(f"- **{model_name}**: 最终={final_acc:.4f}, 最佳={best_acc:.4f}")
        ...
        # 根据数据集规模给出结论
        cnn_accs = [curves_dict[m]['best_val_acc'] for m in cnn_models if m in curves_dict]
        vit_accs = [curves_dict[m]['best_val_acc'] for m in vit_models if m in curves_dict]
            ...
            if dataset_size == "small":
                ...
                if avg_vit >= avg_cnn:
                    ...
                    **原因**：数据充足时，Transformer的强大表达能力开始显现，弱归纳偏置反而成为优势
                    ...
                    st.info("ℹ️ CNN仍然有优势，可能是因为数据量还不够大，或者任务特性更适合CNN")
