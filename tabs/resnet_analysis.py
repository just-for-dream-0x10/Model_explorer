...
def plot_gradient_comparison(plain_stats, resnet_stats):
    ...
    fig.update_layout(title="梯度流对比分析", height=500, showlegend=True)
    ...
    **验证方法**：对比相同深度的普通网络和ResNet，观察梯度流的真实差异
    ...
    with col2:
        if network_type == "简化版（全连接，快速）":
            ...
            num_layers = num_blocks * 2 + 2  # 估算总层数
...
参数量: {info['resnet_params']:,}
    ...
    if st.button("🚀 开始梯度分析", type="primary"):
        ...
        plain_avg = (
            np.mean([stat["mean"] for stat in plain_stats]) if plain_stats else 0
        )
        resnet_avg = (
            np.mean([stat["mean"] for stat in resnet_stats]) if resnet_stats else 0
        )
        plain_vanished = sum(1 for stat in plain_stats if stat["mean"] < 1e-5)
        resnet_vanished = sum(1 for stat in resnet_stats if stat["mean"] < 1e-5)
        ...
        with col2:
            st.metric(
                ...
                f"{resnet_avg:.2e}",
                delta=(
                    f"{((resnet_avg/plain_avg - 1) * 100):.1f}%"
                    if plain_avg > 0
                    else None
                ),
            )
        ...
        with col4:
            st.metric(
                ...
                resnet_vanished,
                delta=(
                    f"{resnet_vanished - plain_vanished}"
                    if plain_vanished > 0
                    else None
                ),
                delta_color="inverse",
        ...
        display_plain = (
            plain_stats[:10] + plain_stats[-10:]
            if len(plain_stats) > 20
            else plain_stats
        )
        ...
        for i, stat in enumerate(display_plain):
            ...
            if mean_grad < 1e-5:
                st.error(
                    f"❌ 层 {i+1}: {stat['layer'][:50]} | 梯度={mean_grad:.2e} (严重消失！)"
                )
            elif mean_grad < 1e-3:
                st.warning(
                    f"⚠️ 层 {i+1}: {stat['layer'][:50]} | 梯度={mean_grad:.2e} (轻微消失)"
                )
            else:
                st.success(
        ...
        display_resnet = (
            resnet_stats[:10] + resnet_stats[-10:]
            if len(resnet_stats) > 20
            else resnet_stats
        )
            ...
            stability_issues.append(
                {
                    ...
                    "icon": "🟡",
                    "severity": "low",
                    "details": {"层数": num_layers, "建议": "增加到30+层效果更明显"},
                    "solution": [
                        "增加到30-50层",
                        "观察更明显的梯度消失现象",
                        "对比效果会更显著",
        ...
        StabilityChecker.display_issues(
            stability_issues, title="🔬 ResNet vs 普通网络 - 梯度稳定性对比"
        )
