...
def plot_layer_memory_bars(layers_info, top_n=10):
    ...
    """
    # 按反向峰值内存排序
    sorted_layers = sorted(layers_info, key=lambda x: x.backward_peak, reverse=True)[:top_n]
    ...
    **分析维度**：前向激活值、反向梯度、参数、峰值内存
    ...
    if analysis_mode == "单层分析":
        ...
        layer_type = st.selectbox(
            "选择层类型",
            ["Conv2d", "Linear", "BatchNorm2d", "MaxPool2d"]
        )
...
参数内存: {info.param_memory:.2f} MB
        ...
        model_choice = st.selectbox(
            "选择模型",
            ["ResNet-18 (简化)", "MobileNet-V2 (简化)", "ViT-Tiny (简化)", "自定义模型"]
        )
        ...
        if st.button("🚀 开始分析", type="primary"):
            ...
            with col4:
                st.metric("峰值内存", f"{summary['peak_memory']:.2f} MB",
                         delta="训练时")
            ...
            if result['layers']:
                ...
                with tab1:
                    fig1 = plot_layer_memory_bars(result['layers'], top_n=min(10, len(result['layers'])))
                ...
                if results[selected_model_for_detail]['layers']:
                    ...
                    for idx, layer in enumerate(layers):
                        ...
                        table_data["输入形状"].append(str(layer.input_shape) if layer.input_shape else "N/A")
                        table_data["输出形状"].append(str(layer.output_shape) if layer.output_shape else "N/A")
                        table_data["参数数量"].append(f"{layer.param_count:,}")
                        table_data["参数内存(MB)"].append(f"{layer.param_memory:.4f}")
                        table_data["激活值内存(MB)"].append(f"{layer.output_memory:.4f}")
...
反向峰值: {selected_layer.backward_peak:.4f} MB
                    ...
                    if selected_layer.layer_type == "Conv2d" and selected_layer.input_shape:
                    ...
                    elif selected_layer.layer_type == "Linear" and selected_layer.input_shape:
                        ...
                        **Linear层内存计算**：
                        ...
                        st.info(f"层类型: {selected_layer.layer_type}，内存占用已计算，详细公式请参考上方的计算依据说明。")
    ...
    **对比不同架构的内存使用差异**：
    ...
    with col3:
        ...
        input_size_compare = st.selectbox("图像尺寸", [112, 224, 384], index=1, key="img_size")
    ...
    if st.button("🚀 开始多模型对比", type="primary", key="multi_model"):
        with st.spinner("分析中..."):
            ...
            for i in range(cnn_layers):
                ...
                cnn_layers_list.append(nn.Conv2d(current_channels, out_channels, 3, stride=stride, padding=1))
        ...
        for model_name, result in results.items():
            ...
            comparison_data["前向内存(MB)"].append(f"{summary['total_forward_memory']:.2f}")
            comparison_data["反向内存(MB)"].append(f"{summary['total_backward_memory']:.2f}")
            comparison_data["参数内存(MB)"].append(f"{summary['total_param_memory']:.2f}")
        ...
        fig_scatter.add_trace(go.Scatter(
            ...
            marker=dict(size=15, color=peak_mem, colorscale='Viridis', showscale=True),
            name='模型'
        ))
        ...
        for model_name in models:
            if param_counts[model_name] > 0:
                efficiency = results[model_name]['summary']['peak_memory'] / param_counts[model_name]
