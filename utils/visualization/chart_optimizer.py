...
class ChartCache:
    ...
    def render_chart(
        ...
        # 渲染图表
        st.plotly_chart(
            fig, use_container_width=use_container_width, width=width, key=key
        )
...
class ChartOptimizer:
    ...
    def optimize_figure(fig: go.Figure, enable_animations: bool = False) -> go.Figure:
        ...
        if not enable_animations:
            fig.update_layout(
                transition_duration=0, hovermode="x" if len(fig.data) > 1 else "closest"
            )
    ...
    @staticmethod
    def create_lightweight_scatter(x_data: list, y_data: list, **kwargs) -> go.Figure:
        """创建轻量级散点图
...
def cached_plotly_chart(
    fig: go.Figure, cache_key_data: Optional[Dict[str, Any]] = None, **kwargs
) -> None:
    """带缓存的图表渲染函数
