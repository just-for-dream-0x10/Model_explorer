...
import pandas as pd
from .chart_optimizer import ChartOptimizer, cached_plotly_chart
...
class ChartBuilder:
    ...
    Example:
        ...
        ...     line_names=["系列1", "系列2"]
        ... )
        >>> cached_plotly_chart(fig, use_container_width=True)
    """
    ...
    def create_line_chart(
        ...
        **kwargs,
        ...
        if not x_data or not y_data:
            raise ValueError("x_data和y_data不能为空")
        ...
        # 检查数据长度匹配
        for y_values in y_data:
            if len(x_data) != len(y_values):
                raise ValueError("x_data和y_data长度必须匹配")
    ...
    def create_heatmap(self, data: np.ndarray, title: str, **kwargs) -> go.Figure:
        ...
        Args:
            data: 2D数据数组
            title: 图表标题
            **kwargs: 其他参数，包括x_title, y_title, colorscale, height等
    ...
    def render_chart_optimized(
        ...
        **kwargs,
        ...
        if use_cache and cache_data:
            ...
            # 直接渲染
            cached_plotly_chart(fig, **kwargs)
