...
class NetworkVisualization:
    ...
    def plot_training_curves(
        self, curves_data: Dict[str, Dict[str, List[float]]], metric: str = "loss"
    ) -> go.Figure:
        """绘制训练曲线
    ...
    def plot_parameter_distribution(
        self, param_data: Dict[str, List[int]], title: str = "参数量分布"
    ) -> go.Figure:
        """绘制参数量分布图
        ...
        fig.update_layout(
            title=title, xaxis_title="网络层", yaxis_title="参数量", height=400
        )

        return fig

    def plot_gradient_flow(
        self, gradient_data: Dict[str, List[float]], title: str = "梯度流分析"
    ) -> go.Figure:
        """绘制梯度流图
...
class MathVisualization:
    ...
    def plot_convolution_process(
        ...
        fig.add_trace(
            go.Heatmap(
                z=input_matrix, colorscale="Blues", showscale=False, name="输入"
            ),
            row=1,
            col=1,
        )
        ...
        # 卷积核
        fig.add_trace(
            go.Heatmap(z=kernel, colorscale="RdBu", showscale=False, name="卷积核"),
            row=1,
            col=2,
        )
        ...
        fig.add_trace(
            go.Heatmap(
                z=output_matrix, colorscale="Viridis", showscale=True, name="输出"
            ),
            row=1,
            col=3,
        )
