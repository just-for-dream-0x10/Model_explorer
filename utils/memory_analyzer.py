...
def analyze_linear_memory(
    in_features: int, out_features: int, input_shape: Tuple[int, int]
) -> LayerMemoryInfo:
    """
    分析Linear层的内存占用
...
def analyze_batchnorm_memory(
    num_features: int, input_shape: Tuple[int, ...]
) -> LayerMemoryInfo:
    """
    分析BatchNorm层的内存占用
...
def analyze_pooling_memory(
    input_shape: Tuple[int, int, int, int], kernel_size: int, stride: int
) -> LayerMemoryInfo:
    """
    分析Pooling层的内存占用
...
def analyze_model_memory(
    model: nn.Module, input_shape: Tuple[int, ...], detailed: bool = True
) -> Dict:
    """
    分析整个模型的内存占用
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d):
            ...
            info = analyze_linear_memory(
                module.in_features, module.out_features, current_shape
            )
            info.name = name
            layers_info.append(info)
            current_shape = info.output_shape
            ...
            info = analyze_batchnorm_memory(module.num_features, current_shape)
            info.name = name
            layers_info.append(info)
    ...
    peak_memory = (
        max(layer.backward_peak for layer in layers_info) if layers_info else 0
    )
    ...
    if layers_info:
        ...
        bottleneck_percentage = (
            (bottleneck_layer.backward_peak / peak_memory * 100)
            if peak_memory > 0
            else 0
        )
    else:
    ...
    result = {
        ...
        "bottleneck": {"layer": bottleneck_layer, "percentage": bottleneck_percentage},
    }
...
if __name__ == "__main__":
    ...
    linear_info = analyze_linear_memory(
        in_features=512, out_features=1000, input_shape=(1, 512)
    )
