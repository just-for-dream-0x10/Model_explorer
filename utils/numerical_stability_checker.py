...
class StabilityChecker:
    ...
    @staticmethod
    def check_activation(activations: np.ndarray, name: str = "激活值") -> Dict[str, Any]:
        """
        检查激活值是否正常
    ...
    @staticmethod
    def check_gate_saturation(gate_values: np.ndarray, gate_name: str = "门控") -> Dict[str, Any]:
        """
        检查门控是否饱和（用于LSTM/GRU）
    ...
    @staticmethod
    def verify_gradient(numerical_grad: np.ndarray, 
                       analytical_grad: np.ndarray,
                       name: str = "梯度") -> Dict[str, Any]:
        """
        ...
        relative_error = diff / (np.abs(numerical_grad).mean() + np.abs(analytical_grad).mean() + 1e-8)
    ...
    def display_issues(issues: List[Dict[str, Any]], 
        ...
        errors = [i for i in issues if i['status'] == 'error' and i.get('severity') != 'critical']
        ...
        if critical:
            ...
            for issue in critical:
                with st.expander(f"{issue['icon']} {issue['type']}: {issue['value']}", expanded=True):
...
def compute_numerical_gradient(neuron, input_data, upstream_gradient, epsilon=1e-5):
    ...
    for i in range(neuron.weights.size):
        # 扁平化索引
        idx = np.unravel_index(i, neuron.weights.shape) if neuron.weights.ndim > 1 else i
        
        # 前向扰动
        original = neuron.weights.flat[i] if neuron.weights.ndim > 1 else neuron.weights[i]
