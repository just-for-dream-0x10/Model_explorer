...
class InsufficientMemoryError(NetworkAnalysisError):
    ...
    def __init__(self, required_memory_mb: float, available_memory_mb: Optional[float] = None):
        """初始化异常
...
class ConfigurationError(NetworkAnalysisError):
    ...
    def __init__(self, config_key: str, config_value: Any, error_details: Optional[str] = None):
        """初始化异常
...
class DataValidationError(NetworkAnalysisError):
    ...
    def __init__(
        ...
        Args:
            ...
            error_details: 错误详情
        """
        message = f"数据验证失败: {data_name} = {actual_value}，期望{expected_condition}"
        if error_details:
            message += f" - {error_details}"
...
class CacheError(NetworkAnalysisError):
    ...
    def __init__(self, operation: str, cache_key: str, error_details: Optional[str] = None):
        """初始化异常
