...
def render_pool_params(
    ...
    with col1:
        kernel_size = st.selectbox(
            "池化核大小",
            [2, 3, 4],
            index=[2, 3, 4].index(default_kernel_size) if default_kernel_size in [2, 3, 4] else 0,
            key=f"{key_prefix}_kernel",
            help="池化窗口的大小"
        )
...
def render_activation_selector(
    ...
    with col1:
        activation_type = st.selectbox(
            ...
            index=["ReLU", "LeakyReLU", "Sigmoid", "Tanh", "GELU", "ELU"].index(default) if default in ["ReLU", "LeakyReLU", "Sigmoid", "Tanh", "GELU", "ELU"] else 0,
...
def _show_conv_summary(params: Dict[str, Any]):
    ...
    with col1:
        receptive_field = params['kernel_size'] + (params['kernel_size'] - 1) * (params.get('dilation', 1) - 1)
        st.metric("感受野", f"{receptive_field}×{receptive_field}")
    
    with col2:
        if params['stride'] == 1 and params['padding'] == (params['kernel_size'] - 1) // 2:
...
def render_layer_params_sidebar():
    ...
    with st.sidebar.expander("🔲 卷积层", expanded=False):
        params['conv'] = render_conv2d_params(
            key_prefix="sidebar_conv",
            show_advanced=False
        )
    
    with st.sidebar.expander("⬇️ 池化层", expanded=False):
        params['pool'] = render_pool_params(
            key_prefix="sidebar_pool"
        )
    ...
    with st.sidebar.expander("🎲 Dropout", expanded=False):
        params['dropout'] = render_dropout_params(
            key_prefix="sidebar_dropout"
        )
