...
def compare_normalization_methods(x, num_groups=32):
    ...
    results["batch_norm"] = {"normalized": bn_normalized, "stats": bn_stats}

    # LayerNorm
    ln_normalized, ln_stats = apply_layer_norm(x.clone())
    results["layer_norm"] = {"normalized": ln_normalized, "stats": ln_stats}

    # GroupNorm
    gn_normalized, gn_stats = apply_group_norm(x.clone(), num_groups)
    results["group_norm"] = {"normalized": gn_normalized, "stats": gn_stats}
