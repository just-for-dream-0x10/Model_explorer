...
from ..exceptions import InvalidLayerConfigError, ComputationError, DataValidationError
...
class NetworkAnalyzer:
    ...
    def __init__(self, input_shape: Tuple[int, ...]):
        ...
        if not input_shape or len(input_shape) < 3:
            raise DataValidationError("input_shape", input_shape, "长度≥3的元组")
    ...
    def analyze_network(self) -> Dict[str, Any]:
        ...
        """
        if not self.layers:
            raise DataValidationError("layers", self.layers, "非空列表")
        ...
        try:
            ...
            raise ComputationError(operation="网络分析", error_details=str(e)) from e
    ...
    def _perform_analysis(self) -> Dict[str, Any]:
        ...
        for i, layer in enumerate(self.layers):
            ...
            layer_details.append(
                {"layer_index": i, "layer_type": layer["layer_type"], **layer_result}
            )
    ...
    def _analyze_layer(
        self, layer: Dict[str, Any], input_shape: Tuple[int, ...]
    ) -> Dict[str, Any]:
        """分析单个层
    ...
    def _analyze_conv2d(
        self, params: Dict[str, Any], input_shape: Tuple[int, ...]
    ) -> Dict[str, Any]:
        """分析卷积层
        ...
        if len(input_shape) != 3:
            raise DataValidationError(
                "conv2d_input_shape", input_shape, "长度为3的元组(C,H,W)"
            )
