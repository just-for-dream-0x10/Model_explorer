...
class MemoryAnalyzer:
    ...
    def _analyze_conv2d_memory(
        ...
        if input_shape:
            ...
            activation_memory = np.prod(output_shape) * self.bytes_per_element / self.BYTES_PER_MB
    ...
    def _analyze_maxpool2d_memory(
        ...
        return {
            ...
            'details': {
                'note': '池化层无参数，仅占用激活值内存',
                'dtype': self.dtype
            }
        }
    
    def _analyze_batchnorm2d_memory(
        self, 
        params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """分析批归一化层内存占用"""
    ...
    def check_memory_feasibility(
        ...
        return {
            ...
            'feasible': available_memory_mb is None or required_memory_mb <= available_memory_mb
        }