...
class TestLayerAnalyzer:
    ...
    def test_linear_analysis_basic(self):
        ...
        result = self.analyzer.linear_analysis(
            in_features=512, out_features=1000, use_bias=True
        )
    ...
    def test_linear_analysis_no_bias(self):
        """测试Linear无偏置情况"""
        result = self.analyzer.linear_analysis(
            in_features=256, out_features=128, use_bias=False
        )
    ...
    def test_attention_analysis_basic(self):
        ...
        result = self.analyzer.attention_analysis(
            d_model=512, num_heads=8, seq_len=128, has_qkv_bias=True
        )
    ...
    def test_attention_analysis_no_bias(self):
        """测试多头注意力无偏置情况"""
        result = self.analyzer.attention_analysis(
            d_model=256, num_heads=4, seq_len=64, has_qkv_bias=False
        )
    ...
    def test_lstm_analysis_bidirectional(self):
        """测试LSTM双向情况"""
        result = self.analyzer.lstm_analysis(
            input_size=64, hidden_size=128, num_layers=1, bias=True, bidirectional=True
        )

        assert result["bidirectional"] is True
        # 双向LSTM参数量应该是单向的2倍
        unidirectional_result = self.analyzer.lstm_analysis(
            input_size=64, hidden_size=128, num_layers=1, bias=True, bidirectional=False
        )
        assert (
            result["parameters"]["total"]
            == 2 * unidirectional_result["parameters"]["total"]
        )

    def test_layernorm_analysis(self):
        """测试LayerNorm分析"""
        result = self.analyzer.layernorm_analysis(
            normalized_shape=512, input_shape=(2, 10, 512)
        )
    ...
    def test_batchnorm2d_analysis(self):
        ...
        result = self.analyzer.batchnorm2d_analysis(
            num_features=64, input_shape=(64, 56, 56)
        )
